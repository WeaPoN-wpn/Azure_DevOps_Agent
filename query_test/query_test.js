// query_test.js
// Node.js script to fetch work items containing "BRITE" in the title
...
async function fetchWorkItemDetails(ids) {
  ...
  console.log(`Starting to fetch ${initialIds.length} BRITE work items...`);
  
  // First pass: fetch initial work items and collect their direct relationships
  for (const id of initialIds) {
    if (fetchedWorkItems.has(id)) continue;
    
    console.log(`Fetching details for BRITE work item ${id}...`);
...
//     const imgFileName = `brite_workitem_${workItemId}_${imageIndex}${ext}`;
...
// Main function to fetch BRITE work items
async function main() {
  try {
    console.log('Starting BRITE work items collection (one level recursion)...');
    console.log(`Organization: ${ORGANIZATION}`);
    console.log(`Project: ${PROJECT}`);
    
    // WIQL query to find work items with "BRITE" in title
    const wiql = {
      query: `SELECT [System.Id], [System.Title], [System.State], [System.AssignedTo] 
              FROM WorkItems 
              WHERE [System.Title] CONTAINS 'BRITE' 
              ORDER BY [System.Id] ASC`
    };
    
    console.log('Querying work items with "BRITE" in title...');
    ...
    console.log(`Found ${ids.length} work items containing "BRITE" in title.`);
    
    if (ids.length === 0) {
      const emptyResult = [];
      fs.writeFileSync(OUTPUT_FILE, JSON.stringify(emptyResult, null, 2), 'utf-8');
      console.log('No BRITE work items found. Empty file created.');
      return;
    }
    
    // Log found work item IDs
    console.log('BRITE Work Item IDs:', ids.join(', '));
    ...
    // Summary
    const briteItems = allWorkItems.filter(item => 
      item.title.toUpperCase().includes('BRITE')
    );
    
    console.log('\n=== SUMMARY ===');
    console.log(`Primary BRITE work items: ${briteItems.length}`);
    console.log(`Total work items (including related): ${allWorkItems.length}`);
    console.log(`Image collection: DISABLED (commented out)`);
    
    // List BRITE work items
    console.log('\n=== BRITE WORK ITEMS ===');
    briteItems.forEach(item => {
